...
export default class Table extends React.Component {
    ...
    getTableHeaders() {
        ...
        return header.reverse().map((key, index) => {
    ...
    getTableData() {
        ...
        return values.reverse().map((key, index) => {
          if(typeof(key[1]) === "object") {
            return this.getLinkButtons(); //<td>links</td>
          }
          else {
            return <td className={'td'} key={index}>{key[1].toString()}</td>
          }
        })
    }
    ...
    getLinkButtons(){
      ...
      if(links != null){
        ...
        return <td className={'td'} key="link">{array}</td>
      }
      else {
        console.log("returned nothing")
        return <td className={'td'} key="link">none</td>
      }
    }
